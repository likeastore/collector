...
function connector(params, state, callback) {
	...
	log.info('preparing request in (' + state.mode + ') mode for user: ' + state.user);
	...
	function formatRequestUri(state) {
		var base = API + params.uri + '&count=200';
		return state.maxId ?
	...
	function handleResponse(response, body) {
		...
		if (Array.isArray(body)) {
			var favorites = params.preprocess(body).map(function (fav) {
				return {
					itemId: fav.id_str,
					user: state.user,
